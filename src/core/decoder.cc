...
#pragma pack(push, 1)
    namespace {
        struct EthHdr {
            [[ maybe_unused]] std::uint8_t dst[6];
            [[ maybe_unused]] std::uint8_t src[6];
            [[ maybe_unused]] std::uint16_t ethertype;
        };
    }
    ...
    namespace {
        struct Ipv4Hdr {
            std::uint8_t ver_ihl;
            [[maybe_unused]] std::uint8_t dscp_ecn;
...
#pragma pack(pop)
    ...
    static bool is_http_port(const std::uint16_t port) {
        switch (port) {
            case 80:
            case 443:
            case 8080:
